...
#[test]
fn ops_empty_with_hardlink_skips_both() {
    let repo_dir = tempfile::tempdir().unwrap();
    let work_dir = tempfile::tempdir().unwrap();
    init_git_repo(repo_dir.path());

    write_file(&work_dir.path().join("f.txt"), b"local");
    write_file(&repo_dir.path().join("f.txt"), b"repo");

    let item = Item {
        path_in_repo: "f.txt".to_string(),
        source: Some(work_dir.path().join("f.txt")),
        is_hardlink: true,
        ops: Ops::new([]),
        devices: HashMap::new(),
    };
    ...
    ops::handle_collect(&config, repo_dir.path(), false, false).unwrap();
    // 仓库文件不变
    assert_eq!(read_file(&repo_dir.path().join("f.txt")), "repo");

    ops::handle_restore(&config, repo_dir.path(), false).unwrap();
    // 本地文件不变
    assert_eq!(read_file(&work_dir.path().join("f.txt")), "local");
}
