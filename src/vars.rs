...
//!      视为未定义，返回错误）
//!    - `{REPO}` —— gsb 仓库根目录的绝对路径
//! 2. **用户自定义变量**：在配置文件 `[vars]` 表中声明，例如：
//!    ```toml
//!    [vars]
//!    my_root = "/some/root"
//!    ```
//!    则 `{my_root}` 即可被解析为 `/some/root`。
//!
//! 使用 `{{` 与 `}}` 转义字面量花括号。
//!
//! 跨设备的差异化路径应通过 `[item.devices.<id>]` 单独配置，而不是依赖
//! 每台设备上不同的 `[vars]`。
        ...
        easy_strfmt::strfmt(path, &self.map)
            ...
            .map_err(|e| match e {
                easy_strfmt::Error::KeyNotFound(key) => {
                    anyhow::anyhow!("未识别的变量: {{{key}}}")
                }
                easy_strfmt::Error::UnmatchedOpenBrace => {
                    anyhow::anyhow!("未闭合的变量引用: '{{' 缺少匹配的 '}}'")
                }
                easy_strfmt::Error::UnmatchedCloseBrace => {
                    anyhow::anyhow!("多余的 '}}'")
                }
                easy_strfmt::Error::WriteError(_) => {
                    anyhow::anyhow!("格式化输出错误")
                }
            })
            .with_context(|| format!("展开路径失败: {path:?}"))
    }
...
pub fn current_device() -> Result<String> {
    utils::get_current_device_name().context("无法获取当前设备 ID")
}
