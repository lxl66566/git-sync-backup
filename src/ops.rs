...
//!
//! 与上一版相比：
//! - 错误统一使用 [`anyhow::Result`]，并在关键位置用 `.context(...)` 附加上下文；
//! - 配置语义基于新的 `ops` + 设备表，旧的 `ignore_*` / `restore_*` 字段已移除；
    ...
    pub fn ask(&mut self, prompt: &str) -> Result<bool> {
        ...
        loop {
            ...
            match line.trim().to_ascii_lowercase().as_str() {
                ...
                _ => {
                    writeln!(stdout, "  输入 y=是 / n=否 / a=全部是 / q=退出")?;
                }
            }
        }
    }
...
fn are_contents_equal(path1: &Path, path2: &Path) -> Result<bool> {
    let mut f1 =
        BufReader::new(fs::File::open(path1).with_context(|| format!("打开文件失败: {path1:?}"))?);
    let mut f2 =
        BufReader::new(fs::File::open(path2).with_context(|| format!("打开文件失败: {path2:?}"))?);
    ...
    loop {
        let n1 = f1
            .read(&mut buf1)
            .with_context(|| format!("读取文件失败: {path1:?}"))?;
        let n2 = f2
            .read(&mut buf2)
            .with_context(|| format!("读取文件失败: {path2:?}"))?;
        if n1 != n2 {
            return Ok(false);
        }
        if n1 == 0 {
...
fn copy_item(from: &Path, to: &Path) -> Result<()> {
    ...
    if from.is_dir() {
        if !to.exists() {
            fs::create_dir(to).with_context(|| format!("创建目录失败: {to:?}"))?;
        }

        let mut src_names = HashSet::new();
        for entry in fs::read_dir(from).with_context(|| format!("读取目录失败: {from:?}"))? {
            let entry = entry.with_context(|| format!("读取目录项失败: {from:?}"))?;
        ...
        for entry in fs::read_dir(to).with_context(|| format!("读取目录失败: {to:?}"))? {
            let entry = entry.with_context(|| format!("读取目录项失败: {to:?}"))?;
            let file_name = entry.file_name();
            if !src_names.contains(&file_name) {
                let dest_path = entry.path();
                if dest_path.is_dir() {
                    fs::remove_dir_all(&dest_path)
                        .with_context(|| format!("删除目录失败: {dest_path:?}"))?;
                } else {
                    fs::remove_file(&dest_path)
                        .with_context(|| format!("删除文件失败: {dest_path:?}"))?;
                }
    ...
    if to.exists() {
        let from_meta = fs::metadata(from).with_context(|| format!("读取元数据失败: {from:?}"))?;
        let to_meta = fs::metadata(to).with_context(|| format!("读取元数据失败: {to:?}"))?;
    ...
    if should_copy {
        ...
        fs::copy(from, to).with_context(|| format!("复制文件失败: {from:?} -> {to:?}"))?;
    } else {
...
///   中断整批同步）
/// - `is_hardlink == true` 但 `from` 不是文件：同上，跳过
...
fn copy_item_all(from: &Path, to: &Path, is_hardlink: bool) -> Result<()> {
    ...
    if is_hardlink {
        ...
        fs::hard_link(from, to).with_context(|| format!("创建硬链接失败: {from:?} -> {to:?}"))?;
    } else {
        copy_item(from, to)?;
    }
    Ok(())
...
fn plan_items(config: &Config, repo_root: &Path, op: Op, vars: &Vars) -> Result<Vec<Plan>> {
    ...
    for item in &config.items {
        ...
        let local_path = vars
            ...
            .with_context(|| format!("展开 item {:?} 的 source 路径失败", item.path_in_repo))?
            .fuck_backslash();
        let repo_path = item.resolve_repo_relative(repo_root).fuck_backslash();
...
pub fn handle_collect(
    ...
    let repo = GsbRepo::open(repo_root).context("打开 git 仓库失败")?;
    ...
    println!("将要 collect 以下 {} 个 item:", plans.len());
    for p in &plans {
    ...
    accepted.par_iter().try_for_each(|p| -> Result<()> {
        ...
        copy_item_all(&p.local_path, &p.repo_path, use_hardlink)
            .with_context(|| format!("collect item {path_in_repo:?} 失败"))
    })?;

    #[cfg(feature = "encrypt")]
    if let Some(plan) = &crypt_plan {
        info!("Encrypting collected files...");
        plan.encrypt().context("加密失败")?;
    }
...
pub fn handle_restore(config: &Config, repo_root: &Path, interactive: bool) -> Result<()> {
    ...
    println!("将要 restore 以下 {} 个 item:", plans.len());
    for p in &plans {
        println!("  {}  ->  {}", p.path_in_repo, p.local_path.display());
    }
    ...
    if !skipped.is_empty() {
        println!("\n跳过 ({} 个):", skipped.len());
        for item in &skipped {
            println!("  {}  (ops 不含 restore)", item.path_in_repo);
        }
    }
    ...
    if let Some(plan) = crypt_plan {
        ...
        accepted.par_iter().try_for_each(|p| -> Result<()> {
            ...
            .with_context(|| format!("restore item {path_in_repo:?} 失败"))
        })?;
...
mod crypt_plan {
    ...
    impl CryptPlan {
        ...
        pub fn build(repo_root: &Path, active_paths: &[&str]) -> Result<Option<Self>> {
            ...
            let key = gse_repo
                .get_key()
                .context("git-simple-encrypt 密钥未配置，但 crypt_list 中存在匹配项")?;
        ...
        pub fn encrypt(&self) -> Result<()> {
            crypt::encrypt_repo(&self.repo, &self.paths).context("加密失败")?;
            Ok(())
        }
    }
...
fn smart_restore_item(src: &Path, dst: &Path, key: &[u8]) -> Result<()> {
    ...
    if src.is_file() {
        if decrypt_file_to(src, dst, key)
            .with_context(|| format!("解密失败: {src:?} -> {dst:?}"))?
            .is_none()
        {
            copy_item(src, dst)?;
        }
        ...
        if !dst.exists() {
            fs::create_dir(dst).with_context(|| format!("创建目录失败: {dst:?}"))?;
        }
        let mut src_names = HashSet::new();
        for entry in fs::read_dir(src).with_context(|| format!("读取目录失败: {src:?}"))? {
            let entry = entry.with_context(|| format!("读取目录项失败: {src:?}"))?;
            let name = entry.file_name();
            src_names.insert(name.clone());
            smart_restore_item(&entry.path(), &dst.join(&name), key)?;
        }
        for entry in fs::read_dir(dst).with_context(|| format!("读取目录失败: {dst:?}"))? {
            let entry = entry.with_context(|| format!("读取目录项失败: {dst:?}"))?;
            let name = entry.file_name();
            if !src_names.contains(&name) {
                let path = entry.path();
                if path.is_dir() {
                    fs::remove_dir_all(&path).with_context(|| format!("删除目录失败: {path:?}"))?;
                } else {
...
mod tests {
    ...
    use super::*;
    use crate::config::{Config, DeviceOverride, GitConfig, Item, Ops};
    use crate::vars::Vars;
