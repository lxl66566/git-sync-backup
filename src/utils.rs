...
pub fn get_current_device_name() -> Result<String> {
    machine_uid::get()
        .map_err(|e| anyhow::anyhow!("Could not get current device ID (machine-uid): {e}"))
}

/// 从当前目录开始向上查找 `.gsb.config.toml` 所在目录，作为仓库根目录。
pub fn find_repo_root() -> Result<PathBuf> {
    let current_dir = env::current_dir().context("Could not get current working directory")?;
        ...
        match current_path.parent() {
            ...
            None => {
                return Err(anyhow::anyhow!(
                    "Could not find .gsb.config.toml: no gsb config file in the current directory or any parent directory"
                ));
            }
        }
    }
